...
public interface RestaurantsRepository extends JpaRepository<Restaurants, Long> {
	...
	//@Query(value =" select Distinct(r.restaurant_id) From Restaurants as r where r.id IN(:foodIds) ", nativeQuery = true)
	@Query(value ="select Distinct(restaurant_id) From restaurants_food where food_id IN(:foodIds) ", nativeQuery = true)
	List<Long> getRestaurantIdsByFoodIds(List<Long> foodIds);

	@Query(value =" select * From restaurants where id IN(:restaurantIds)", nativeQuery = true)
